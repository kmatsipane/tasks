...
export function isCorrect(question: Question, answer: string): boolean {
    const question1: string = question.expected;
    return question1.toLowerCase().trim() === answer.toLowerCase().trim();
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    const truth: boolean = question.options.some(
        (phrase: string): boolean => phrase === answer,
    );
    return truth;
}
...
export function toShortForm(question: Question): string {
    const q: string = question.id + ": " + question.name.slice(0, 10);
    return q;
}
...
export function toMarkdown(question: Question): string {
    let words: string[] = question.options.map(
        (word: string): string => "- " + word,
    );
    let ans: string = words.join("\n");
    if (ans === "") {
...
export function addOption(question: Question, newOption: string): Question {
    const words: string[] = [...question.options, newOption];
    return { ...question, options: words };
}
